...
AddEvent("OnTranslationReady", function()
    ...
    policeMenu = Dialog.create(_("police_menu"), nil, _("handcuff_player"), _("put_player_in_vehicle"), _("remove_player_from_vehicle"), _("remove_all_weapons"),_("give_player_fine"),_("search_police"), _("cancel"))
    ...
    Dialog.addTextInput(policeFineMenu, 1, _("fine_price").." :")
    Dialog.addSelect(policeFineMenu, 1, _("player"), 3)
    Dialog.addTextInput(policeFineMenu, 1, _("reason").." :")
    
    policeSearchMenu = Dialog.create(_("search_police"), nil, _("start_search"), _("cancel"))
    Dialog.addSelect(policeSearchMenu, 1, _("player"), 3)
end)
...
AddEvent("OnDialogSubmit", function(dialog, button, ...)
    ...
    if dialog == policeMenu then
        ...
        if button == 6 then
            CallRemoteEvent("OpenPoliceSearchMenu")
        end
    ...
    if dialog == policeSearchMenu then
        if button == 1 then
            if args[1] ~= "" then
                CallRemoteEvent("SearchPlayerInventory", args[1])
            else
                MakeNotification(_("select_someone"), "linear-gradient(to right, #ff5f6d, #ffc371)")
            end
        end
    end
...
AddRemoteEvent("OpenPoliceSearchMenu", function(playerNames)
    print("OpenPoliceSearchMenu")
    print(playerNames)
    Dialog.setSelectLabeledOptions(policeSearchMenu, 1, 2, playerNames)
    Dialog.show(policeSearchMenu)
end)
